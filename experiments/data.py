...
def mimicicu_dataLoad(data_dir):
    gc.collect()
    global cols_orig
    orig = pd.read_csv(data_dir,header=0)
    print(len(orig))
    '''
...
def activity_dataLoad_pt(data_dir):
    global cols_orig
    if  device == torch.device("cpu"):
        orig = torch.load(data_dir,map_location=torch.device('cpu')) 
    else:
        orig = torch.load(data_dir)  
    print(orig)
    df_orig =  pd.DataFrame(orig,columns=['file_id','a1','b2','c3'])
    #df_orig = pd.read_csv(data_dir,header=0,na_filter=True)  
    #print(df_orig)
    ...
    data_category = data_dir.split('/')[2]
    data = './data/'+data_category

    df_value = pd.DataFrame(b2)
    df_value.to_csv(data+'/activity_value.csv')
    for col in df_value.columns:
    ...
    #print(df_value.columns)
    df=df_value[[0,1,2,3,4,5]]

    
    for col in df.columns:
        col_name = 'activity_'+str(col)
        df[col_name]=df[col]
        df=df.drop(col,axis=1)
