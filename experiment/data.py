...
scaler = MinMaxScaler()
def airquality_dataLoad(data_path,opt):
    global cols_orig
        ...
        for i in range(len(df)):
            try:
                if df.loc[i,col]==-9999:
                    ...
                    df.loc[i,col]  =np.mean(df.loc[i-8:i-1,col])
    '''
    randint = random.randint(0,len(df)-opt.poolSize+1)
    
    orig = df[-randint-opt.poolSize:-randint]
    print(orig)
    ...
    return orig
def mimicicu_dataLoad(data_path,opt):
    global cols_orig
    df_orig = pd.read_csv(data_path,header=0)    
    ...
    #print(len(data),len(x_orig),len(y_orig))
    return orig
def ecg_dataLoad(data_path,opt):
    global cols_orig
    ...
    #print(len(data),len(x_orig),len(y_orig))
    return orig
def uci_dataLoad(train_data_path,test_data_path,opt):
    global cols_orig
    df_train = pd.read_csv(train_data_path,header=0,na_filter=True)  
    print(df_train.columns) 
    
    df_test = pd.read_csv(test_data_path,header=0,na_filter=True)  
...
def eeg_dataLoad(data_path,opt):
    global cols_orig
    df_orig = pd.read_csv(data_path,header=0,na_filter=True)  
    print(df_orig.columns) 
